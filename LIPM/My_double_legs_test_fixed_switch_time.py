...
for i in range(1,data_len+1):
    ...
    if i<=switch_index:
        LIPM_model.capture_point=LIPM_model.updateCapturePoint(LIPM_model.xt,LIPM_model.vt,
                                                                LIPM_model.target_orbital_energy)
    ...
    if((i>=switch_index)and(i%switch_index==0)):
        ...
        xt , vt =LIPM_model.updateXtVt(LIPM_model.x0,LIPM_model.v0,
                                       LIPM_model.delta_t*(switch_index))
        ...
        # interpolation the reference foot trajectory of the swing leg from current to the
        # next capture point
        if LIPM_model.support_leg=='left_leg':
            swing_foot_reference_x=np.linspace(LIPM_model.right_foot_x,LIPM_model.left_foot_x+capture_point,
                                               switch_index)
        else:
            swing_foot_reference_x=np.linspace(LIPM_model.left_foot_x,LIPM_model.right_foot_x+capture_point,
                                               switch_index)
        j=0
        ...
        orbital_energy_array.append(LIPM_model.right_foot_z)
