...
class LIPM3D:
    ...
    def calculateFootLocationForNextStep(self, s_x=0.0, s_y=0.0, a=1.0, b=1.0, theta=0.0, x_0=0.0, vx_0=0.0, y_0=0.0, vy_0=0.0):
        ...
        print('当前的支撑腿位置（世界坐标系下）：')
        print('-- p_x_current=%.3f'%self.p_x, ', p_y_current=%.3f'%self.p_y)
        p_x_new, p_y_new = self.nextReferenceFootLocation(s_x, s_y, theta)
        print('根据步行参数计算下一步的参考落脚点（世界参考系坐标下）：')
        ...
        print('下一步的参考落脚点支撑结束后的质心位置和速度（轨迹对称轴参考坐标系下）：')
        ...
        print('下一步的参考落脚点支撑结束后的质心位置和速度（世界参考坐标系下）：')
