...
for i in range(int(total_time/delta_t)):
    ...
    if (i > 0) and (i % switch_index == 0):
        ...
        print('切换支撑腿之后，再经过一个支撑周期T_sup时间，质心的位置（支撑腿坐标系）：x_ex_after_sup=%.3f'%x_0,', vx_ex_after_sup=%.3f'%y_0,
              ', y_ex_after_sup=%.3f'%y_0,', vy_ex_after_sup=%.3f'%vy_0)
        ...
        print('切换支撑腿之后，再经过一个支撑周期T_sup时间，质心的位置（世界坐标系）：x_ex_after_sup=%.3f'%x_0,', vx_ex_after_sup=%.3f'%y_0,
              ', y_ex_after_sup=%.3f'%y_0,', vy_ex_after_sup=%.3f'%vy_0)
