...
class My_LIPM:
    ...
    # update the capture point associated with the terget orbital energy
    def updateCapturePoint(self,xt,vt,target_OE):
        if(vt**2<2*target_OE):
            capture_point=self.capture_point
            print("warning: calculate the capture point failed, the velocity is too low. capture_point=",capture_point)
        else:
...
if __name__=='__main__':
    ...
    z0=1.0
    delta_t=0.01
    LIPM_model = My_LIPM(x0,v0,z0,delta_t)
